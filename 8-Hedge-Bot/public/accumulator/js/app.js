...
class App {
  ...
  init() {
    ...
    // Populate credential inputs now that DOM is ready
    this.populateCredentialInputs();

    // Auto-connect if saved token exists, otherwise show modal
    const savedToken = localStorage.getItem('deriv_api_token') || '';
    if (savedToken && savedToken.trim().length > 5) {
      this.log('info', 'Saved credentials found. Auto-connecting to Deriv...');
      this.wsClient.connect().catch(err => {
        this.log('error', `Auto-connect failed: ${err.message}. Please re-enter API Keys.`);
  ...
  loadSavedCredentials() {
    ...
    let savedAppId = localStorage.getItem('hedge_app_id') || localStorage.getItem('deriv_app_id') || '';
    if (!savedAppId || savedAppId.trim().length === 0) {
      savedAppId = defaultAppId;
    }
    const savedToken = localStorage.getItem('hedge_api_token') || localStorage.getItem('deriv_api_token') || '';
    localStorage.setItem('hedge_app_id', savedAppId);
    localStorage.setItem('deriv_app_id', savedAppId);
    if (savedToken) {
      localStorage.setItem('hedge_api_token', savedToken);
      localStorage.setItem('deriv_api_token', savedToken);
    }
    this.wsClient.setCredentials(savedAppId, savedToken);
  }

  populateCredentialInputs() {
    const savedAppId = localStorage.getItem('hedge_app_id') || localStorage.getItem('deriv_app_id') || '1089';
    const savedToken = localStorage.getItem('hedge_api_token') || localStorage.getItem('deriv_api_token') || '';
