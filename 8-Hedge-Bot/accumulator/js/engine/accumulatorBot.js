...
export class AccumulatorBotEngine {
  ...
  stop(reason = 'User Requested') {
    ...
    // Clean up server-side subscriptions to prevent AlreadySubscribed on restart
    this.client.forgetAllSubscriptions().catch(() => {});
  ...
  triggerNextTrade() {
    ...
    if (!this.isImmediateNextBuy && this.state !== 'WAITING_FOR_CRASH') {
      ...
      this._lastStatsString = undefined;
      this._crashBuyTriggered = false;
      this._initialStatsLen = null;
      this._seenNonZeroRun = false;
      this.emit('stateChange', { state: this.state, isRunning: true });
  ...
  onProposalReceived(proposal) {
    ...
    // Establish baseline on very first proposal packet after state change
    if (this._initialStatsLen === null) {
      this._initialStatsLen = stats.length;
      if (currentRunTicks > 0) {
        this._seenNonZeroRun = true;
      }
      return; // Skip initial packet to avoid stale 0 from previous crash
    }

    // Track active market ticks
    if (currentRunTicks > 0) {
      this._seenNonZeroRun = true;
    }

    // Trigger buy ONLY on a genuine NEW crash (0 reached after observing active ticks or array length growth)
    const isNewCrash = (currentRunTicks === 0) && (this._seenNonZeroRun || stats.length > this._initialStatsLen);
